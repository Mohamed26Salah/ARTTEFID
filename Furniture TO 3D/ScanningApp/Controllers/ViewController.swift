...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate, UIDocumentPickerDelegate {
    ...
    //Variable of scan
    var boxToScan : BoundingBox!
    var assetNew : MDLAsset?
    var anchorCount = 0
    var isButtonPressed = false
    //Variables of scan end
...
//            if boxToScan.contains(meshAnchor.transform.position){
...
//    }
    ...
    func scanPercentageChanged(_ notification: Notification) {
        ...
        if percentage > 50 {
            DispatchQueue.main.async { [self] in
                exportButton.setTitle("Fuck Joex", for: .normal)
                exportButton.sizeToFit()
                exportButton.center = view.center
                exportButton.addTarget(self, action: #selector(buttonTapped), for: .touchUpInside)
    ...
    func boundingBoxPositionOrExtentChanged(_ notification: Notification) {
        ...
        boxToScan = notification.object as? BoundingBox
...
//            configuration.sceneReconstruction = .mesh
   ...
   func scanInsideBox(anchor: ARMeshAnchor) -> Bool{
       let minExtentX = boxToScan.simdWorldPosition.x - (boxToScan.extent.x)
       let minExtentY = boxToScan.simdWorldPosition.y - (boxToScan.extent.y)
       let minExtentZ = boxToScan.simdWorldPosition.z - (boxToScan.extent.z)
       let maxExtentX = boxToScan.simdWorldPosition.x + (boxToScan.extent.x)
       let maxExtentY = boxToScan.simdWorldPosition.y + (boxToScan.extent.y)
       let maxExtentZ = boxToScan.simdWorldPosition.z + (boxToScan.extent.z)
       
//       let minExtent = -boxToScan.extent / 2 + boxToScan.simdWorldPosition
//       let maxExtent = boxToScan.extent / 2 + boxToScan.simdWorldPosition
//       return (minExtent.x...maxExtent.x).contains(anchor.transform.position.x) &&
//           (minExtent.y...maxExtent.y).contains(anchor.transform.position.y) &&
//           (minExtent.z...maxExtent.z).contains(anchor.transform.position.z)
       let anchorTransform = anchor.transform
       let anchorPosition = SCNVector3(anchorTransform.columns.3.x, anchorTransform.columns.3.y, anchorTransform.columns.3.z)
       
       
       if ((minExtentX <= anchorPosition.x && anchorPosition.x <= maxExtentX) &&
           (minExtentY <= anchorPosition.y && anchorPosition.y <= maxExtentY) &&
           (minExtentZ <= anchorPosition.z && anchorPosition.z <= maxExtentZ)) {
           return true
       }
               return false
   }
    @objc func buttonTapped(_ sender: UIButton) {
        func convertToAsset(meshAnchors: [ARMeshAnchor]) -> MDLAsset? {
            ...
            for anchor in meshAnchors {
                print(anchor.geometry.classification ?? "None")
           //    if (anchor.geometry.scanInsideBox(anchor: anchor, modelMatrix: anchor.transform, boxToScan: boxToScan)){
//                if(boxToScan.contains2(anchor.transform.position)){
//                if(scanInsideBox(anchor: anchor)){
                    let mdlMesh = anchor.geometry.toMDLMesh(device: device,  modelMatrix: anchor.transform, boxToScan: boxToScan)
                    asset.add(mdlMesh)
                print("Extent\(boxToScan.extent)")
                print("World Position\(boxToScan.simdWorldPosition)")
             //   }
//                }
...
//             }
        ...
        func wait3dScan(meshAnchors : [ARMeshAnchor] ,completion: @escaping (_ success: Bool) -> Void){
            assetNew = convertToAsset(meshAnchors: meshAnchors)
            let success = true
            completion(success)
        }
        ...
        if var meshAnchors = sceneView.session.currentFrame?.anchors.compactMap({ $0 as? ARMeshAnchor }) {
            wait3dScan(meshAnchors: meshAnchors)
            { [weak self] success in
                if success {
                    do {
                        let url = try export(asset: self!.assetNew!)
                        share(url: url)
