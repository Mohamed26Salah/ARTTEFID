...
class RoomEnv(Env):
    def __init__(self,FurnitureDimensions,DoorsDim , DoorsPos,WindowsDim,WindowsPos , keywords , RoomDimension):
        ...
        self.observation_space = np.zeros((1,len(keywords),2)) 
        # Set start temp
        values=[random.randint(0,RoomDimension[0]),random.randint(0,RoomDimension[1])]
        self.state2 =[[random.randint(0,RoomDimension[0]), random.randint(0,RoomDimension[1])] for _ in range(len(keywords))]
    ...
    def step(self, action):
        ...
        return self.state2, reward, done, info
...
#         pygame.event.pump()
    ...
    def reset(self):
        ...
        self.state2 =[[random.randint(0,self.RoomDimension[0]), random.randint(0,self.RoomDimension[1])] for _ in range(len(self.keywords))]
        self.state=self.state2[self.turn]
